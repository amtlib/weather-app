...
const mapApiDataToWeatherEntries = (apiData: any): WeatherEntry[] => {
    return apiData.properties.timeseries.map((timeseries) => {
        const details = timeseries.data.instant.details;
        const time = dayjs(timeseries.time)
        if(time < dayjs()) return undefined;
    
        return { time: time, pressure: details.air_pressure_at_sea_level, temperature: details.air_temperature, humidity: details.relative_humidity, windDirection: details.wind_from_direction, windSpeed: details.wind_speed, symbolCode: timeseries.data.next_1_hours?.summary.symbol_code};
    }).filter(e => e !== undefined);
}

const WeatherContext = createContext<WeatherContextType>({ location: undefined, weather: [] })
...
export const WeatherProvider = ({children}: PropsWithChildren) => {
    ...
    const getWeather = useCallback(async () => {
        if (!location) return;
        const { data } = await axios.get(getWeatherEndpoint(location), {headers: { "User-Agent": "grzegorz.pach@protonmail.com"}});
